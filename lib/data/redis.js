...
db.prototype = {
	...
	createBackup: function(room) {
		for (let i=9; i >= 0; i--){
			if (i !== 0){
				redisClient.del(REDIS_PREFIX + '-room:' + room + '-cards-' + (i+1))
				redisClient.hgetall(REDIS_PREFIX + '-room:' + room + '-cards-' + i, function(err, res){
					if (res !== null){
						for (id in res) {
				...
				redisClient.del(REDIS_PREFIX + '-room:' + room + '-cards-' + 1)
				redisClient.hgetall(REDIS_PREFIX + '-room:' + room + '-cards', function(err, res){
					for (id in res) {
						redisClient.hset(REDIS_PREFIX + '-room:' + room + '-cards-' + 1, id, res[id])
					}
				})
