...
db.prototype = {
	...
	getAllCardsBackup: function(room, num, callback) {
		...
		redisClient.keys(REDIS_PREFIX + '-room:' + room + '-cards*', function (err, res){
			listKeyBackup = res

			if (listKeyBackup.includes(key)){
	...
	createBackup: function(room, numBackup) {
		if (numBackup !== 0){
			...
			for (let u=10; u>= numBackup; u--){
				redisClient.hgetall(REDIS_PREFIX + '-room:' + room + '-cards-' + u, function(err, res){
					if (res !== null){
						if ((u - numBackup) !== 0){
							for (id in res) {
								redisClient.hset(REDIS_PREFIX + '-room:' + room + '-cards-' + (u - numBackup), id, res[id])
							}
						} else {
							for (id in res) {
		...
		for (let i=9; i >= 0; i--){
			...
			redisClient.hgetall(REDIS_PREFIX + '-room:' + room + '-cards' + num, function(err, res){
				if (res !== null){
					...
					redisClient.hset(REDIS_PREFIX + '-room:' + room + '-cards-' + (i + 1), 0, "null")
				}
			})
		}
